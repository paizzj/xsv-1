...
static bool SignContractStep(const BaseSignatureCreator &creator,
                     ...
                     const CScript &scriptPubKey,
                     std::vector<valtype> &ret,
                     txnouttype &whichTypeRet, bool role) {
    CScript scriptRet;
    uint160 h160;
    ret.clear();
    ...
    CScript sc = CScript();
    if (role) {
        sc = CScript(scriptPubKey.begin() + 1, scriptPubKey.begin() + 26);
    } else {
        sc = CScript(scriptPubKey.begin() + 27, scriptPubKey.begin() + 52);
    }
...
bool ProduceContractSignature(const Config& config, bool consensus, const BaseSignatureCreator& creator, bool genesisEnabled, bool utxoAfterGenesis,
                      const CScript& fromPubKey, SignatureData& sigdata, bool role) {
    CScript script = fromPubKey;
    bool solved = true;
    std::vector<valtype> result;
    txnouttype whichType;
